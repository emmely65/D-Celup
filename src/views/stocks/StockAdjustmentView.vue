...
const materials = ref([])
const form = reactive({ material_id: '', actual_stock: 0, movement_date: new Date().toISOString().slice(0, 10), note: '' })
const isLoading = ref(false)

onMounted(async () => { materials.value = unwrapList(await rawMaterialApi.getAll({ per_page: 100 })) })

async function submit() {
  // BUG-12c: Validasi client-side sebelum submit
  ...
  try {
    ...
    uiStore.showToast('success', 'Koreksi stok berhasil dicatat')
    Object.assign(form, { material_id: '', actual_stock: 0, movement_date: new Date().toISOString().slice(0, 10), note: '' })
  ...
  <DashboardLayout>
    ...
    <section class="mt-4 max-w-2xl rounded-xl border border-dcelup-border bg-dcelup-creamSoft p-4">
      <div class="grid gap-3 md:grid-cols-2">
        ...
        <BaseInput v-model="form.note" label="Catatan" class="md:col-span-2" />
      </div>
